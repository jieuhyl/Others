...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 23 17:58:58 2019
...
clf_xgb = xgb.XGBClassifier(random_state= 1337)
...
y_pred1 = clf_xgb.predict(xgb.DMatrix(X_train), ntree_limit=clf_xgb.best_iteration)
y_pred2 = clf_xgb.predict(xgb.DMatrix(X_test), ntree_limit=clf_xgb.best_iteration)
...
clf_xgb = xgb.XGBClassifier(learning_rate=0.1, n_estimators=60, objective='multi:softmax', num_class=4, silent=1, verbosity = 2)
...
for n_fold, (train_idx, valid_idx) in enumerate(folds.split(X_train, y_train)):
        ...
        #clf_xgb = xgb.train(**xgb_params)
        clf_xgb.fit(train_x, train_y, 
                eval_set = [(train_x, train_y), (valid_x, valid_y)], eval_metric = 'merror', 
...
for i in range(0, 10):
    ...
    clf_xgb = xgb.XGBClassifier(**params)
    clf_xgb.fit(indep_train, dep_train, 
                eval_set=[(indep_train, dep_train), (indep_test, dep_test)], 
                eval_metric = 'merror', 
                verbose=False, 
                early_stopping_rounds=50)
    model.append(clf_xgb)
    cvscores.append(accuracy_score(y_test, clf_xgb.predict(X_test)))
